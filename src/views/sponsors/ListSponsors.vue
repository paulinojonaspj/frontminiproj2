          ...
          <table class="table table-striped">
            ...
            <tbody>
              <tr v-for="sponsor of sponsors" :key="sponsor._id">
                ...
                <td class="pt-4">{{sponsor.nacionalidade}}</td>
                <td class="pt-4">{{sponsor.empresa}}</td>
                <td>
                  <router-link
                  ...
                  <button
                    @click="viewSponsors(sponsor._id)"
                    type="button"
                    class="btn btn-outline-success mr-2 mt-2"
                  >
                  ...
                  <button
                    @click="removeSponsors(sponsor._id)"
                    type="button"
...
<script>
import { FETCH_SPONSORS, REMOVE_SPONSORS } from "@/store/sponsors/sponsors.constants";
...
export default {
  ...
  methods: {
    ...
    generateTemplate(sponsor) {
      let response = `
          <p><b>Nome:</b> ${sponsor.nome}<br>Nacionalidade: ${sponsor.nacionalidade}<br>Empresa: ${sponsor.empresa}</p>
    ...
    removeSponsors(id) {
      this.$confirm(
        ...
        () => {
          this.$store.dispatch(`sponsor/${REMOVE_SPONSORS}`, id).then(() => {
            this.$alert(
              this.getMessage,
              "Sponsor removido!",
              "success"
